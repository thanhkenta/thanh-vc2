<template>
  <el-dialog
    title="Chỉnh sửa cấu hình thời gian"
    :visible.sync="editSettingTimeFormVisible"
    width="500px"
  ...
  >
    ...
    <span slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" size="small" @click="handleAdd" :disabled="loading">Lưu</el-button>
    </span>
  </el-dialog>
</template>
...
<script>
  ...
  export default {
    props: ['editSettingTimeFormVisible', 'selectedTimeSetting'],
    data() {
    ...
    watch: {
      selectedTimeSetting(setting) {
        this.teamInfo = Object.assign({}, setting.info_team)
        this.form.days = setting.day
        this.form.timeRange = [setting.time_from, setting.time_to]
        if (setting.from_date) {
          this.form.advancedSetting = true
          this.form.dateRange = [setting.from_date, setting.to_date]
        } else {
          this.form.advancedSetting = false
          this.form.dateRange = ''
        }
      }
    },
    methods: {
      handleClose() {
        this.$emit('closeEditSettingTimeModal')
      },
      handleAdd() {
        this.$refs.addSettingTimeForm.validate(valid => {
          if (valid) {
            const params = {
              id: this.selectedTimeSetting.id,
              id_department: this.selectedTimeSetting.id_department,
            ...
            createOrUpdateTimeSetting(params)
              .then(res => {
                ...
                if (res.data.status) {
                  ...
                  this.$emit('settingTimeUpdated')
                } else {
                  this.$notify({
                    dangerouslyUseHTMLString: true,
                    title: 'Cập nhật cài đặt thất bại',
