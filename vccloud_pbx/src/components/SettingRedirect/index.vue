<template>
  <el-dialog
    :title="`Thêm cấu hình điều hướng cuộc gọi (${greetingType === 'department' ? selectedDepartment.department_name : 'khoảng thời gian'})`"
  ...
  >
    <el-form :model="form" label-width="200px" label-position="top">
      ...
      <el-form-item v-if="form.selectedRedirectMode === 'team'" label="Chọn team">
        <el-row>
          <el-col :span="18" style="position: relative">
            <el-input disabled v-model="form.teamName"></el-input>
          </el-col>
          <el-col :span="6">
...
<script>
  ...
  export default {
    ...
    computed: {
      selectedPhone() {
        return this.selectedRedirectNumber.phoneNumber ? `${this.selectedRedirectNumber.phoneNumber} (${this.selectedRedirectNumber.name})` : ''
      }
    },
    watch: {
      selectedTeam(team) {
        this.form.teamName = team.team_name
      }
    },
    methods: {
      handleSubmit() {
        this.$confirm('Bạn chắc chắn muốn lưu cài đặt này?', 'Chú ý', {
          ...
          bindOrUpdateForward(data)
            .then(res => {
              if (res.data.status) {
                this.$emit('updateTimeRedirectCall')
                this.$notify({
                  title: 'Thành công',
                  message: 'Lưu cấu hình chuyển tiếp cuộc gọi thành công!',
                  type: 'success'
                })
              } else {
                this.$notify({
                  title: 'Thất bại',
                  message: 'Lưu cấu hình chuyển tiếp cuộc gọi thất bại!',
                  type: 'error'
                })
              }
            })
      ...
      handleClose() {
        ...
        this.$emit('closeRedirectCallModal')
      }
    }
  }
</script>
